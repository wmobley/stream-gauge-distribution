...
const MapComponent = () => {
  ...
  // Update legend config when data or visualization type changes
  useEffect(() => {
    if (geoJsonData && !loading) {
      const currentVisualization = visualizationOptions.find(opt => opt.value === visualizationType);
      if (currentVisualization) {
        const ranges = calculateDataRanges(geoJsonData, currentVisualization.valueField);
        if (ranges) {
          setLegendConfig({
            title: `${currentVisualization.label} Legend`,
            description: `Based on ${currentVisualization.valueField} values (decile distribution)`,
            visualizationType,
            ranges: ranges.ranges.map((range, index) => ({
              color: range.color,
  ...
  }, [geoJsonData, loading, visualizationType]);
  ...
  useEffect(() => {
    ...
    // Add/Update GeoJSON data when available
    if (mapRef.current && geoJsonData && !loading && legendConfig) {
      ...
      // Get data ranges for color calculation
      const dataRanges = calculateDataRanges(geoJsonData, currentVisualization.valueField);
      ...
      if (filteredFeatures.length > 0 && visualizationType === 'risk') {
        mapRef.current.fitBounds(visualizationLayer.getBounds(), { padding: [20, 20] });
      }
    }

    // Cleanup function - only cleanup when component unmounts
    return () => {
      // Don't cleanup the map here, only cleanup data layers
    };
  }, [geoJsonData, loading, visualizationType, legendConfig]);
